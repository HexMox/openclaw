...
# Install uv (Python package manager) to shared location accessible to all users.
# uv manages its own Python installations, eliminating the need for system Python.
ENV UV_HOME=/usr/local/share/uv
ENV UV_PYTHON_INSTALL_DIR=/usr/local/share/uv/python
RUN curl -LsSf https://astral.sh/uv/install.sh | UV_INSTALL_DIR="$UV_HOME/bin" sh
ENV PATH="${UV_HOME}/bin:${UV_PYTHON_INSTALL_DIR}/bin:${PATH}"

# Install Python via uv (managed Python, no system packages needed).
# Wrapper scripts use 'uv run' to automatically find the managed Python.
...
ENV PNPM_HOME="/home/node/.openclaw/pnpm"
